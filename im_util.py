...
def get_cropped_input(inputImage, bbox, padScale, outputSize):
    ...
    if imagePatch.shape[0] == 0 or imagePatch.shape[1] == 0:
        ...
        scalingFactor_X = patch.shape[0] / imagePatch.shape[0];
        scalingFactor_Y = patch.shape[1] / imagePatch.shape[1];
        print ('scaling factor ',np.round(outputSize * boundedBoxWH[0] / boxOnWH[0]), ' and ',np.round(outputSize * boundedBoxWH[1] / boxOnWH[1]))
        print (' size of image patch ',imagePatch.shape)
        print (' size of resize patch ',patch.shape)
        imt0 = cv2.normalize(patch.astype('float32'), None, 0.0, 1.0, cv2.NORM_MINMAX)
        #cv2.rectangle(imt0, (rect0[0], rect0[1]), (rect0[2], rect0[3]), [255, 0, 0], 2)
        cv2.imshow('image', imt0)
        k = cv2.waitKey(0)
        if k == 27:
        ...
        newWidth = width*scalingFactor_X
        newHeight = height*scalingFactor_Y
        outputBox[0] = bbox[0]+pad[0]
        outputBox[1] = bbox[1] + pad[1]
        outputBox[2] = outputBox[0]+newWidth
        outputBox[3] = outputBox[1]+newHeight


    return patch, outputBox.astype(int)
